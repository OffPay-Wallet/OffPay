...
interface ValidateAgenticPrivateSendParams {
  input: unknown;
  userText?: string;
...
export type AgenticPrivateSendValidation =
  | {
      ok: true;
      draft: Omit<
...
function hasPrivateSendCapabilities(
  capabilities: CapabilitiesResponse['capabilities'] | null | undefined,
): boolean {
  const currentCapabilities = capabilities ?? null;
  return (
    isOffpayFeatureAvailable(currentCapabilities, 'payment.privateInitMint') &&
    isOffpayFeatureAvailable(currentCapabilities, 'payment.privateSend') &&
    isOffpayFeatureAvailable(currentCapabilities, 'payment.rpcBroadcast')
  );
}

export function validateAgenticPrivateSendDraft(
  params: ValidateAgenticPrivateSendParams,
): AgenticPrivateSendValidation {
  if (params.walletAddress == null) {
    return { ok: false, message: 'Connect a wallet before using Yuga private send.' };
  }
  ...
  if (params.walletMode !== 'online' || !params.canUseNetwork) {
    return { ok: false, message: 'Yuga private send needs online wallet mode.' };
  }

  if (params.capabilities == null) {
    return { ok: false, message: 'Private send capability checks are still loading.' };
  }

  if (!hasPrivateSendCapabilities(params.capabilities)) {
    return { ok: false, message: 'Private send is not available on the current network.' };
  }
  ...
  if (tokenText.length === 0) {
    return { ok: false, message: 'Tell me whether to send USDC or USDT.' };
  }
  ...
  if (
    ...
    return { ok: false, message: 'Yuga private send only supports USDC or USDT.' };
  }
  ...
  if (!isAmountWithinBalance(rawAmount, balanceRaw)) {
    return { ok: false, message: `Insufficient ${token.symbol} balance for this private send.` };
  }
