...
- EAS builds need the public client config used by the app: `EXPO_PUBLIC_OFFPAY_API_ORIGIN`, `EXPO_PUBLIC_OFFPAY_API_ALLOWED_ORIGINS`, `EXPO_PUBLIC_OFFPAY_AI_PROXY_URL`, `EXPO_PUBLIC_OFFPAY_AI_PROXY_ALLOWED_ORIGINS`, `EXPO_PUBLIC_OFFPAY_ATTESTATION_MODE`, `EXPO_PUBLIC_GOOGLE_CLOUD_PROJECT_NUMBER`, `EXPO_PUBLIC_OFFPAY_INVITE_GATE_MODE`, `EXPO_PUBLIC_PRIVY_APP_ID`, and `EXPO_PUBLIC_PRIVY_CLIENT_ID`.
- The Android-only production profile builds an APK. Without Play Console enrollment, keep `EXPO_PUBLIC_OFFPAY_ATTESTATION_MODE=play_integrity`, enable Play Integrity in Google Cloud, and configure the API Worker with `OFFPAY_ANDROID_DISTRIBUTION_MODE=sideload` plus the exact EAS signing-certificate SHA-256 fingerprint in `OFFPAY_ANDROID_CERTIFICATE_SHA256_DIGESTS`. `OFFPAY_ANDROID_MIN_VERSION_CODE` blocks older signed APKs. This fail-closed path accepts only the pinned package/certificate on a device returning `MEETS_DEVICE_INTEGRITY`; it does not enable the prototype bypass.
